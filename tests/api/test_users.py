...
# ---- 401: POST /users also requires a token ----
...
# ---- 422: malformed POST /users requests ----


def test_users_create_rejects_missing_body(client: TestClient, token: str) -> None:
    resp = client.post(
        "/users",
        headers={"Authorization": f"Bearer {token}"},
    )
...
def test_users_list_reflects_newly_created_user(client: TestClient, token: str) -> None:
    headers = {"Authorization": f"Bearer {token}"}
...
def test_users_list_returns_json_content_type(client: TestClient, token: str) -> None:
    resp = client.get("/users", headers={"Authorization": f"Bearer {token}"})
